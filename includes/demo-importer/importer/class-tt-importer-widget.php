<?php
/**
 * Class for the widget importer used in the Themebeez Demo Importer plugin.
 *
...
class TT_Importer_Widget {
	...
	private function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new WP_Error(
				...
				__( 'Widget import file could not be found.', 'themebeez-toolkit' )
			);
		}

		// Get file contents and decode.
		$data = TT_Helpers::data_from_file( $file );
	...
	private function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new WP_Error(
				...
				__( 'Widget import data could not be read. Please try a different file.', 'themebeez-toolkit' )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'themebeez-toolkit' ); // Explain why widget not imported.
				}
				...
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'themebeez-toolkit' ); // Show "No Title" if widget instance is untitled.
	...
	public function format_results_for_log( $results ) {
		...
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'themebeez-toolkit' );
		}
