...
class TT_Helpers {
	...
	public static function verify_ajax_call() {
		...
		check_ajax_referer( 'tt-ajax-verification', 'security' );
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ) {
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'themebeez-toolkit' ),
				$log_file_path,
				esc_html__( 'Upload files', 'themebeez-toolkit' )
			);
		}
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'themebeez-toolkit' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'themebeez-toolkit' )
		);
